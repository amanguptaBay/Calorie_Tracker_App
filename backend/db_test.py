...
class TestSimpleMealEntries(unittest.TestCase):
    ...
    def test_create_simple_meals(self):
        ...
        journal.addMeal(data_models.Meal(name = "Breakfast"))
        journal.addMeal(data_models.Meal(name = "Lunch"))
        journal.addMeal(data_models.Meal(name = "Snack"))
        journal.addMeal(data_models.Meal(name = "Dinner"))
    ...
    def test_add_simple_foods(self):
        ...
        journal = db.get_daily_journal("2023-01-01")
        journal.addMeal(data_models.Meal(name = "Breakfast"))
        journal.getMealByName("Breakfast").addEntry(data_models.MealEntry(type = data_models.MealEntryType.FOOD, foodOrMeal = data_models.Food(name = "Extra-Large Eggs", quantity = 2, unit = "eggs", calories = 140)))
        journal.getMealByName("Breakfast").addEntry(data_models.MealEntry(type = data_models.MealEntryType.FOOD, foodOrMeal = data_models.Food(name = "Spinach", quantity = 32, unit = "grams", calories = 10)))
        journal.getMealByName("Breakfast").addEntry(data_models.MealEntry(type = data_models.MealEntryType.FOOD, foodOrMeal = data_models.Food(name = "Tomato", quantity = 1.5, unit = "tomatos", calories = 20)))
        db.push_daily_journal(journal)
        self.assertEquals(db.get_daily_calories("2023-01-01")["Breakfast"], 170)
